...
void initialisationdumodeetduclock(void){
	ME.MER.R=0x0000001D;
...
void enleverwatchdog(void){
	SWT.SR.R=0x0000C520;
	SWT.SR.R=0x0000D928;
	SWT.CR.R=0x8000010A;
}

void fairefonctionnerl_interuption(void){
	INTC.CPR.B.PRI=0;
	asm("wrteei 1");
}
...
/*
void initialisationdutimerPIT(uint32_t Timeout ){
...
int main(void) {
  ...
  //faire_fonctionner_l_interuption();
  //initialisation_du_timer_PIT(1600000);
  //INTC_InstallINTCInterruptHandler(interruption,59,15);
  startChannelPIT(0);
  //PIT_actif();
